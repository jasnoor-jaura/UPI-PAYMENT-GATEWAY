...
$sqlSelectOrderskayupi = "SELECT * FROM orders WHERE byteTransactionId=?";
$stmtSelectOrderskayupi = $conn->prepare($sqlSelectOrderskayupi);
$stmtSelectOrderskayupi->bind_param("s", $byteTransactionId);
$stmtSelectOrderskayupi->execute();
$resultSelectOrders = $stmtSelectOrderskayupi->get_result();
$kayupirrowOrders = $resultSelectOrders->fetch_assoc();
$stmtSelectOrderskayupi->close();
...
$order_id = $kayupirrowOrders['order_id'];
$bytehackamount=$kayupirrowOrders['amount'];
$bytepaytmtxnref=$kayupirrowOrders['paytm_txn_ref']; //byte trans
...
$kayupiremark1 = $rowOrders['remark1'];
    ...
    if ($responseArray['STATUS'] == "TXN_SUCCESS" && $responseArray['MID'] == $mid && $responseArray['ORDERID'] == $txn_ref_id) {
      ...
      //  echo "<pre>" . json_encode($responseArray, JSON_PRETTY_PRINT) . "</pre>";
       $transactionId= $responseArray['TXNID'];
        ...
        $kayupimerchantTransactionId=$responseArray['ORDERID'];
...
// Assuming $kayupimerchantTransactionId and $transactionNote are strings.
$stmtInsertReport->bind_param("sssssssssss", $transactionId, $paymentState, $order_id, $vpa, $user_name, $paymentApp, $amount, $user_token, $transactionNote, $kayupimerchantTransactionId, $megabyteuserid);
...
if ($db_status == 'TXN_SUCCESS' && $kayupimerchantTransactionId==$db_merchantTransactionId && $bytehackamount== $db_amount) {
